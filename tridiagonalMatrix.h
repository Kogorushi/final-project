...
/  CS 5201 Section 1a
/  assignment 6
/  DESC:  a header file for a tridiagonalMatrix class that acts
...
template <typename T>
class tridiagonalMatrix : public virtual matrixInt<T>
{
  private:
  T* m_Matrix;
  int m_numRows;
  ...
  //Pre:  none.
  //Post:  initializes m_numRows and m_numCols to 0 and m_flg to 20.
  tridiagonalMatrix();
  ...
  //       Otherwise, sets m_numRows and m_numCols to 0.  Also, sets m_flg to 20.
  ...
  //       a full matrix into an tridiagonalMatrix)
  tridiagonalMatrix(matrixInt<T>& x);
  //Pre:  none.
  //Post:  If reference to matrixInt x does not reference *this, overwrites the current
  //       object, *this, to be a deep copy of the upper right part of x. (can be used to copy
  //       a full matrix into an tridiagonalMatrix)
  tridiagonalMatrix(tridiagonalMatrix<T>& x);
  //Pre:  none.
  ...
  tridiagonalMatrix& operator=(const matrixInt<T>&);
  //Pre:   assignment must be defined for objects of type T.
  //Post:  if reference to maatrixInt x does not reference *this matrix, overwrite this object
  //       to be a deep copy of x.  Returns a reference to this object.
  ...
  tridiagonalMatrix& buildNew(const int row, const int col);
};
