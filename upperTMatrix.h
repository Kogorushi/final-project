...
/  CS 5201 Section 1a
/  assignment 5
/  DESC:  a header file for a upperTMatrix class that acts
/         like a mathematical matrix.
*/

#ifndef UPPER_T_MATRIX_H
#define UPPER_T_MATRIX_H
#include "matrixInterface.h"

template <typename T>
class upperTMatrix : public virtual matrixInt<T>
{
  protected:
  T* m_Matrix;
  int m_numRows;
  ...
  upperTMatrix(matrixInt<T>& x);
  //Pre:  none.
  //Post:  If reference to upperTMatrix x does not reference *this, overwrites the current
  //       object, *this, to be a deep copy of the upper right part of x. (can be used to copy
  //       a full matrix into an upperTMatrix)
  upperTMatrix(upperTMatrix<T>& x);
  //Pre:  none.
  ...
  upperTMatrix& buildNew(const int row, const int col);
};

#include "upperTMatrix.hpp"
#endif